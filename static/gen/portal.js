...
return true;}}};
APP.URLS={};APP.URLS={getQueryParams:function(forGoogle ){var IGNORE_PARAM_LIST=["north","south","east","west","resultType","source","nawqa_project","project_code"];var ignoreList=(forGoogle)?IGNORE_PARAM_LIST.concat(["mimeType","zip"]):IGNORE_PARAM_LIST;var result=getFormQuery($(APP.DOM.form),ignoreList);if(forGoogle){if(!result){result+='&';}
result+="mimeType=kml";}
...
_gaq.push(['_trackEvent','Portal Map','MapCreate',decodeURIComponent(APP.URLS.getQueryParams()),parseInt(count)]); $('#show-on-map-button').attr('disabled','disabled').removeClass('query-button').addClass('disable-query-button');var formParams=getFormValues($('#params'),['north','south','east','west','resultType','source','project_code','nawqa_project','mimeType','zip','__ncforminfo']);PORTAL.portalDataMap.showDataLayer(formParams,function(){$('#show-on-map-button').removeAttr('disabled').removeClass('disable-query-button').addClass('query-button');});});

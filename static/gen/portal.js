...
var finalParams=$.extend({},defaultParams,params);var finalOptions=$.extend({},defaultOptions,options);var sldDeferred=$.Deferred();$.ajax({url:Config.NWIS_SITE_SLD_URL,dataType:'text',success:function(data){finalOptions.sld_body=data;sldDeferred.resolve(new OpenLayers.Layer.WMS('NWIS Stream Gages','http://cida.usgs.gov/nwc/proxy/geoserver/NWC/wms',finalOptions,finalParams));},error:function(){sldDeferred.resolve(new OpenLayers.Layer.WMS('NWIS Stream Gages','http://cida.usgs.gov/nwc/proxy/geoserver/NWC/wms',finalOptions,finalParams))}});return sldDeferred;}
...
OpenLayers.Control.GetFeature.prototype.selectBox.apply(this,[position]);},selectClick:function(evt){this.selectBoundingBox=this.pixelToBounds(evt.xy);this.selectBoxFeatures=[];OpenLayers.Control.GetFeature.prototype.selectClick.apply(this,[evt]);}});function SitesLayer(layerName ,enableBoxId ,selectFeatureFnc ){this._isBoxIDEnabled=enableBoxId;this._selectFeatureFnc=selectFeatureFnc;this.dataLayer=new OpenLayers.Layer.WMS('Sites',Config.GEOSERVER_PROXY_ENDPOINT+'wms',{layers:layerName,styles:'zoom_based_point',format:'image/png',transparent:true},{displayInLayerSwitcher:false,isBaseLayer:false,transitionEffect:'resize',singleTile:true,visibility:true,opacity:0.75});this._createIdControl=function(){
var dataLayer=this.dataLayer.clone();var protocol=OpenLayers.Protocol.WFS.fromWMSLayer(dataLayer);this.idFeatureControl=new WQPGetFeature({protocol:protocol,box:this._isBoxIDEnabled,click:!this._isBoxIDEnabled,clickTolerance:5,single:false,maxFeatures:50});this.idFeatureControl.events.register('featuresselected',this,function(ev){this._selectFeatureFnc(ev,this.idFeatureControl.getLastBoundingBox());});};this.idFeatureControl=null;this.addToMap=function(map ){map.addLayer(this.dataLayer);this._createIdControl();map.addControl(this.idFeatureControl);this.idFeatureControl.activate();};this.removeFromMap=function(map ){this.idFeatureControl.deactivate();map.removeControl(this.idFeatureControl);map.removeLayer(this.dataLayer);};this.enableBoxId=function(map ,on ){if(this._isBoxIDEnabled!==on){this._isBoxIDEnabled=on;this.idFeatureControl.deactivate();map.removeControl(this.idFeatureControl);this._createIdControl();map.addControl(this.idFeatureControl);this.idFeatureControl.activate();}};};SiteImportWPSUtils={getRequestXML:function(identifier ,dataInputs ){var formattedData=[];for(var i=0;i<dataInputs.length;i++){formattedData.push({'identifier':dataInputs[i].name,'data':{'literalData':{'value':dataInputs[i].value}}});}
...
this.map.addLayers(baseLayers);MapUtils.getNWISSitesLayer().then(function(layer){this.map.addLayer(layer);}.bind(this));this.map.zoomTo(3);var center=new OpenLayers.LonLat(MapUtils.DEFAULT_CENTER.lon,MapUtils.DEFAULT_CENTER.lat);this.map.setCenter(center.transform(MapUtils.WGS84_PROJECTION,MapUtils.MERCATOR_PROJECTION));this.toggleBoxId=function(){if(this.dataLayer){this.dataLayer.enableBoxId(this.map,this._boxIdentifyOn);}};this.cancelIdentifyOp=function(){this._selectBoxLayer.destroyFeatures();};this.cancelMapping=function(){window.clearInterval(this.timerId);this.timerId={};this._updateDivEl.hide();if(this.completeFnc){this.completeFnc();this.completeFnc=null;}};this.fetchDataLayer=function(formParams ,complete ){this.completeFnc=complete;this._updateDivEl.find('span').html('Transferring data, please be patient');this._updateDivEl.show();var theseFormParams=formParams;var thisPortalDataMap=this;OpenLayers.Request.POST({url:Config.GEOSERVER_PROXY_ENDPOINT+'ows?identifier=gs:SiteImport',data:SiteImportWPSUtils.getRequestXML('gs:SiteImport',formParams),success:function(data){

this.statusRequest={status:200};thisPortalDataMap.timerId=window.setInterval(function(){if(this.statusRequest.status){this.statusRequest=OpenLayers.Request.POST({url:Config.GEOSERVER_PROXY_ENDPOINT+'ows?identifier=gs:SingleWpsStatus',data:SiteImportWPSUtils.getRequestXML('gs:SingleWpsStatus',[{name:'layerName',value:data.responseText}]),success:function(data){processStatus(data,thisPortalDataMap,theseFormParams);},failure:function(data){processServiceFailure(thisPortalDataMap,'Unable to contact map server for status: '+data.status);}});}},1000);},failure:function(data){processServiceFailure(thisPortalDataMap,'Unable to contact map server with status: '+data.status);}});};};function processStatus(responseData ,portalDataMap,formParams){
var response=$.parseJSON(responseData.responseText);var updateEl=portalDataMap._updateDivEl.find('span');switch(response.requestStatusType){case'RECEIVED':case'BLOCKED':case'STARTED':case'LOADING_COUNT':updateEl.html('Transferring data, please be patient');break;case'LOADING_VALUES':if(response.visitedRecords){updateEl.html('Constructing data layer '+response.percentComplete+'%');}
else{updateEl.html('Transferring data, please be patient');}
break;case'BUILDING_LAYER':updateEl.html('Building map layer, please be patient');break;case'COMPLETE':portalDataMap.cancelMapping(); if(portalDataMap.dataLayer){portalDataMap.dataLayer.removeFromMap(portalDataMap.map);portalDataMap._selectBoxLayer.destroyFeatures();portalDataMap.map.removeLayer(portalDataMap._selectBoxLayer);}
if(portalDataMap._identifyDialog.dialogEl.dialog('isOpen')){portalDataMap._identifyDialog.dialogEl.dialog('close');}
if(portalDataMap._popupIdentify){portalDataMap.map.removePopup(portalDataMap._popupIdentify);}
portalDataMap.dataLayer=new SitesLayer(response.layerName,portalDataMap._boxIdentifyOn,function(ev,selectBoundingBox){var siteIds=[];var degreeBBox;var i;portalDataMap._updateSelectBoxLayer();
 for(i=0;i<ev.features.length;i++){siteIds.push(ev.features[i].attributes.id);}
if($('body').width()<750){PORTAL.CONTROLLER.retrieveSiteIdInfo(siteIds,function(html){portalDataMap._popupIdentify=new OpenLayers.Popup.FramedCloud('idPopup',selectBoundingBox.getCenterLonLat(),null,html,null,true,function(){portalDataMap._popupIdentify=null;portalDataMap.cancelIdentifyOp();this.destroy();});portalDataMap.map.addPopup(portalDataMap._popupIdentify,true);});}
else{degreeBBox=selectBoundingBox.transform(MapUtils.MERCATOR_PROJECTION,MapUtils.WGS84_PROJECTION);portalDataMap._identifyDialog.updateAndShowDialog(siteIds,degreeBBox,formParams);}});portalDataMap.dataLayer.addToMap(portalDataMap.map);portalDataMap.map.addLayer(portalDataMap._selectBoxLayer);break;default:portalDataMap.cancelMapping();alert('Unable to get data for map: '+response.requestStatusDescription);}};function processServiceFailure(portalDataMap,msg ){alert(msg);portalDataMap.cancelMapping();};var PORTAL=PORTAL||{};PORTAL.VIEWS=PORTAL.VIEWS||{};PORTAL.VIEWS.createStaticSelect2=function(el ,ids ,select2Options ){var defaultOptions={placeholder:'All',allowClear:true};var selectHtml='';var i;for(i=0;i<ids.length;i++){selectHtml+='<option value="'+ids[i]+'">'+ids[i]+'</option>';}
...
_gaq.push(['_trackEvent','Portal Map','MapCreate',decodeURIComponent(APP.URLS.getQueryParams()),parseInt(count)]); $('#show-on-map-button').attr('disabled','disabled').removeClass('query-button').addClass('disable-query-button');PORTAL.portalDataMap.fetchDataLayer(getFormValues($('#params'),['north','south','east','west','resultType','source','mimeType','zip','__ncforminfo']),function(){$('#show-on-map-button').removeAttr('disabled').removeClass('disable-query-button').addClass('query-button');});});

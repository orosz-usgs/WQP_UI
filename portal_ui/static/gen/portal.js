...
else{var paramStr=param.name+':'+param.value.replace(/;/g,'|');result.push(paramStr);}});if(providerValues.length>0){result.push('providers:'+providerValues.join('|'));}
...
el.append(selectHtml);el.select2($.extend({},defaultOptions,select2Options));};PORTAL.VIEWS.createPagedCodeSelect=function(el,spec,select2Options){spec.pagesize=(spec.pagesize)?spec.pagesize:20;if(!('formatData'in spec)){spec.formatData=function(data){var desc=(data.hasOwnProperty('desc')&&(data.desc)?data.desc:data.value);return desc+' ('+PORTAL.MODELS.providers.formatAvailableProviders(data.providers)+')';};}
var defaultOptions={placeholder:'All',allowClear:true,multiple:true,separator:';',formatSelection:function(object,container){return object.id;},ajax:{url:Config.CODES_ENDPOINT+'/'+spec.codes,dataType:'json',data:function(term,page){return{text:term,pagesize:spec.pagesize,pagenumber:page,mimeType:'json'};},quietMillis:250,results:function(data,page,query){var results=[];$.each(data.codes,function(index,code){results.push({id:code.value,text:spec.formatData(code)});});return{results:results,more:((spec.pagesize*page)<data.recordCount)};}}};el.select2($.extend(defaultOptions,select2Options));};PORTAL.VIEWS.createCodeSelect=function(el ,spec ,select2Options ){ if(!('isMatch'in spec)){spec.isMatch=function(data,searchTerm){if(searchTerm){return(data.desc.toUpperCase().indexOf(searchTerm.toUpperCase())>-1);}else{return true;}};}
...
var defaultOptions={placeholder:'All',allowClear:true,multiple:true,separator:';',formatSelection:function(object,container){return object.id;},query:function(options){spec.model.processData(spec.getKeys()).done(function(data){var i,key;var results=[];var dataArray=[];if(length in data&&data.length>0){dataArray=dataArray.concat(data);}else{for(key in data){if((data[key])&&data[key].length>0){dataArray=dataArray.concat(data[key]);}}}
for(i=0;i<dataArray.length;i++){if(spec.isMatch(dataArray[i],options.term)){results.push(spec.formatData(dataArray[i]));}}
options.callback({'results':results});});}};el.select2($.extend(defaultOptions,select2Options));};var PORTAL=PORTAL||{};PORTAL.onReady=function(){"use strict";var placeSelects;var select2Options={};PORTAL.portalDataMap=undefined;PORTAL.downloadProgressDialog=PORTAL.VIEWS.downloadProgressDialog($('#download-status-dialog'));PORTAL.MODELS.providers.initialize().done(function(){PORTAL.VIEWS.createStaticSelect2($('#providers-select'),PORTAL.MODELS.providers.getIds());}).fail(function(error){alert('Unable to retrieve provider list with error: '+error);});placeSelects=PORTAL.VIEWS.placeSelects($('#countrycode'),$('#statecode'),$('#countycode')); PORTAL.VIEWS.createCodeSelect($('#siteType'),{model:PORTAL.MODELS.siteType},select2Options);PORTAL.VIEWS.createCodeSelect($('#organization'),{model:PORTAL.MODELS.organization,formatData:function(data){return{id:data.id,text:data.id+' - '+data.desc};},isMatch:function(data,searchTerm){if(searchTerm){var searchTermUpper=searchTerm.toUpperCase();return((data.id.toUpperCase().indexOf(searchTermUpper)>-1)||(data.desc.toUpperCase().indexOf(searchTermUpper)>-1));}
else{return true;}}},$.extend({},select2Options,{closeOnSelect:false,minimumInputLength:2}));PORTAL.VIEWS.createCodeSelect($('#sampleMedia'),{model:PORTAL.MODELS.sampleMedia},select2Options);PORTAL.VIEWS.createCodeSelect($('#characteristicType'),{model:PORTAL.MODELS.characteristicType},select2Options);PORTAL.VIEWS.createPagedCodeSelect($('#characteristicName'),{codes:'characteristicname'},$.extend({},select2Options,{closeOnSelect:false}));PORTAL.VIEWS.createPagedCodeSelect($('#subject-taxonomic-name'),{codes:'subjecttaxonomicname'},$.extend({},select2Options,{closeOnSelect:false}));PORTAL.VIEWS.createCodeSelect($('#assemblage'),{model:PORTAL.MODELS.assemblage},select2Options);PORTAL.VIEWS.createPagedCodeSelect($('#project-code'),{codes:'project'},$.extend({},select2Options,{closeOnSelect:false})); PORTAL.VIEWS.inputValidation({inputEl:$('#siteid'),validationFnc:PORTAL.validators.siteIdValidator});PORTAL.VIEWS.inputValidation({inputEl:$('#startDateLo'),validationFnc:PORTAL.dateValidator.validate,updateFnc:function(value){return PORTAL.dateValidator.format(value,true);}});PORTAL.VIEWS.inputValidation({inputEl:$('#startDateHi'),validationFnc:PORTAL.dateValidator.validate,updateFnc:function(value){return PORTAL.dateValidator.format(value,false);}});PORTAL.VIEWS.inputValidation({inputEl:$('#bounding-box input[type="text"]'),validationFnc:PORTAL.validators.realNumberValidator});PORTAL.VIEWS.inputValidation({inputEl:$('#point-location input[type="text"]'),validationFnc:PORTAL.validators.realNumberValidator});PORTAL.VIEWS.inputValidation({inputEl:$('#huc'),validationFnc:PORTAL.hucValidator.validate,updateFnc:PORTAL.hucValidator.format});$('html').click(function(e){$('.popover-help').popover('hide');});$('.popover-help').each(function(){var options=$.extend({},PORTAL.MODELS.help[($(this).data('help'))],{html:true,trigger:'manual'});$(this).popover(options).click(function(e){$(this).popover('toggle');e.stopPropagation();});}); $('.panel-heading .show-hide-toggle').click(function(){toggleShowHideSections($(this),$(this).parents('.panel').find('.panel-body'));});$('.subpanel-heading .show-hide-toggle').click(function(){toggleShowHideSections($(this),$(this).parents('.subpanel').find('.subpanel-body'));});

var mapBox=$('#query-map-box');var mapDiv=$('#query-results-map');mapDiv.height(mapBox.height());$(window).resize(function(){if(mapBox.height()!==mapDiv.height()){mapDiv.height(mapBox.height());}}); $('#show-on-map-button').click(function(){var fileFormat='xml';var showMap=function(totalCount){ if($('#query-map-box').is(':hidden')){$('#mapping-div .show-hide-toggle').click();}
_gaq.push(['_trackEvent','Portal Map','MapCreate',decodeURIComponent(PORTAL.queryServices.getQueryParams()),parseInt(totalCount)]); $('#show-on-map-button').attr('disabled','disabled').removeClass('query-button').addClass('disable-query-button');var formParams=getFormValues($('#params'),['north','south','east','west','resultType','source','project_code','nawqa_project','mimeType','zip','__ncforminfo']);PORTAL.portalDataMap.showDataLayer(formParams,function(){$('#show-on-map-button').removeAttr('disabled').removeClass('disable-query-button').addClass('query-button');});};if(!PORTAL.CONTROLLERS.validateDownloadForm()){return;}
_gaq.push(['_trackEvent','Portal Map','MapCount',decodeURIComponent(PORTAL.queryServices.getQueryParams())]);PORTAL.downloadProgressDialog.show('map');PORTAL.queryServices.getHeadRequest('Station').done(function(response){var counts=PORTAL.DataSourceUtils.getCountsFromHeader(response,PORTAL.MODELS.providers.getIds());PORTAL.downloadProgressDialog.updateProgress(counts,'Station',fileFormat,showMap);}).fail(function(message){PORTAL.downloadProgressDialog.cancelProgress(message);});}); $('#main-button').click(function(event){var startDownload=function(totalCount){_gaq.push(['_trackEvent','Portal Page',$('#params input[name="resultType"]').val()+'Download',decodeURIComponent(PORTAL.queryServices.getQueryParams()),parseInt(totalCount)]);$('#params').submit();};var fileFormat=$('input[name="mimeType"]').val();var resultType=$('#params input[name="resultType"]').val();if(!PORTAL.CONTROLLERS.validateDownloadForm()){return;}
event.preventDefault();_gaq.push(['_trackEvent','Portal Page',resultType+'Count',decodeURIComponent(PORTAL.queryServices.getQueryParams())]);PORTAL.downloadProgressDialog.show('download');PORTAL.queryServices.getHeadRequest(resultType).done(function(response){var counts=PORTAL.DataSourceUtils.getCountsFromHeader(response,PORTAL.MODELS.providers.getIds());PORTAL.downloadProgressDialog.updateProgress(counts,resultType,fileFormat,startDownload);}).fail(function(message){PORTAL.downloadProgressDialog.cancelProgress(message);});}); function updateMyLocation(){if(navigator.geolocation&&navigator.geolocation.getCurrentPosition){navigator.geolocation.getCurrentPosition(updateFormLocation,displayError,{timeout:8000,maximumAge:60000});}else{alert("Sorry! your browser does not support geolocation.");}
return false;}
...
$('#download-box input[name="mimeType"]:radio').click(function(){var sensitive=!($('#download-box #kml').prop('checked'));setEnabled($('#download-box #samples'),sensitive);setEnabled($('#download-box #biosamples'),sensitive);$('#params input[name="mimeType"]:hidden').val($(this).val());}); $('#download-box input[name=resultType]').click(function(){var $form=$('#params');var $biosamplesRadio=$('#download-box #biosamples');var sensitive=!($('#download-box #samples').prop('checked'))&&!($biosamplesRadio.prop('checked'));setEnabled($('#download-box #kml'),sensitive);$form.attr('action',PORTAL.queryServices.getFormUrl($(this).val()));$form.find('input[name="resultType"]:hidden').val($(this).val());if($biosamplesRadio.prop('checked')){if($form.find('input[name="dataProfile"]').length===0){$form.append('<input type="hidden" name="dataProfile" value="biological" />');}}else{$form.find('input[name="dataProfile"]').remove();}}); $('#sorted').change(function(){var $hidden=$('#params input[name="sorted"]');if($(this).is(':checked')){$hidden.val('yes');}
else{$hidden.val('no');}}); $('#bounding-box input').change(function(){var north=$('#north').val();var south=$('#south').val();var east=$('#east').val();var west=$('#west').val();var bboxVal='';if((north)&&(south)&&(east)&&(west)){bboxVal=west+','+south+','+east+','+north;}
$('#params input[name="bBox"]').val(bboxVal);}); $('#project-code, #nawqa-project').change(function(){var nawqaValues='';var projectCodes=[];var projectCodeValues;var d=$('#project-code').select2('data');$.each($('#project-code').select2('data'),function(index,el){projectCodes.push(el.id);});projectCodeValues=projectCodes.join(';');if($('#nawqa-project').is(':checked')){nawqaValues+=Config.NAWQA_ONLY_PROJECTS;}
if(nawqaValues!==''&&projectCodeValues!==''){nawqaValues+=';';}
$('#params input[name="project"]').val(nawqaValues+projectCodeValues);}); $('#show-queries-button').click(function(){
var stationSection="<div class=\"show-query-text\"><b>Sites</b><br><textarea readonly=\"readonly\" rows='6'>"+PORTAL.queryServices.getFormUrl('Station')+"</textarea></div>";var resultSection="<div class=\"show-query-text\"><b>Results</b><br><textarea readonly=\"readonly\" rows='6'>"+PORTAL.queryServices.getFormUrl('Result')+"</textarea></div>";$('#WSFeedback').html(stationSection+resultSection);}); var identifyDialog=new IdentifyDialog('map-info-dialog',PORTAL.queryServices.getFormUrl); $('#mapping-div .show-hide-toggle').click(function(){var isVisible=toggleShowHideSections($(this),$('#query-map-box'));var boxIdToggleEl=$('#map-identify-tool');if(isVisible){setEnabled(boxIdToggleEl,true);if(!PORTAL.portalDataMap){PORTAL.portalDataMap=new PortalDataMap('query-results-map','map-loading-div',identifyDialog);$('#cancel-map-button').click(function(){PORTAL.portalDataMap.cancelMapping();});}}
else{setEnabled(boxIdToggleEl,false);}}); $('#map-identify-tool').click(function(){PORTAL.portalDataMap.toggleBoxId();});};var PORTAL=PORTAL||{};PORTAL.MODELS=PORTAL.MODELS||{};PORTAL.MODELS.providers=function(){var ids=[];return{ initialize:function(){var deferred=$.Deferred();$.ajax({url:Config.CODES_ENDPOINT+'/providers',data:{mimeType:'json'},type:'GET',success:function(data,textStatus,jqXHR){ids=[];$.each(data.codes,function(index,code){ids.push(code.value);});deferred.resolve();},error:function(jqXHR,textStatus,error){ids=[];deferred.reject(error);}});return deferred.promise();},getIds:function(){return ids;},formatAvailableProviders:function(availableProviders ){var availableList=availableProviders.split(' ');var resultList=[];var i;for(i=0;i<ids.length;i++){if($.inArray(ids[i],availableList)!==-1){resultList.push(ids[i]);}}
if(resultList.length===0){return null;}
else if(resultList.length===ids.length){return'all';}
else{return resultList.join(', ');}}};}();var PORTAL=PORTAL||{};PORTAL.MODELS=PORTAL.MODELS||{};PORTAL.MODELS.cachedCodes=function(spec){var that={};var cachedData=[];var ajaxCalled=false;var ajaxCompleteDeferred=$.Deferred();that.processData=function(){if(!ajaxCalled){ajaxCalled=true;if(ajaxCompleteDeferred.state()==='rejected'){ ajaxCompleteDeferred=$.Deferred();}
$.ajax({url:Config.CODES_ENDPOINT+'/'+spec.codes,type:'GET',data:{mimeType:'json'},success:function(data,textStatus,jqXHR){$.each(data.codes,function(index,code){cachedData.push({id:code.value,desc:(code.hasOwnProperty('desc')&&(code.desc)?code.desc:code.value),providers:code.providers});});ajaxCompleteDeferred.resolve(cachedData);},error:function(jqXHR,textStatus,error){alert('Can\'t  get '+spec.codes+', Server error: '+error);ajaxCompleteDeferred.reject(error);ajaxCalled=false;}});}
return ajaxCompleteDeferred.promise();};return that;};PORTAL.MODELS.cachedCodesWithKeys=function(spec){var that={};var cachedData={};var getData=function(keys ){var results={};var i=0;for(i=0;i<keys.length;i++){results[keys[i]]=cachedData[keys[i]];}
return results;};that.processData=function(keys){var keysToGet=[];var i;var ajaxData={};var deferred=$.Deferred(); for(i=0;i<keys.length;i++){if(!(keys[i]in cachedData)){keysToGet.push(keys[i]);}}
if(keysToGet.length===0){ deferred.resolve(getData(keys));}
else{ajaxData[spec.keyParameter]=keysToGet.join(';');ajaxData.mimeType='json';$.ajax({url:Config.CODES_ENDPOINT+'/'+spec.codes,type:'GET',data:ajaxData,success:function(data,textStatus,jqXHR){var k; for(k=0;k<keysToGet.length;k++){cachedData[keysToGet[k]]=[];}
$.each(data.codes,function(index,code){var thisData={id:code.value,desc:(code.hasOwnProperty('desc')&&(code.desc)?code.desc:code.value),providers:code.providers};var key=spec.parseKey(thisData.id);cachedData[key].push(thisData);});deferred.resolve(getData(keys));},error:function(jqXHR,textStatus,error){alert("Can't get "+spec.codes+', Server error: '+error);deferred.reject(error);}});}
return deferred.promise();};return that;};PORTAL.MODELS.countryCodes=PORTAL.MODELS.cachedCodes({codes:'countrycode'});PORTAL.MODELS.stateCodes=PORTAL.MODELS.cachedCodesWithKeys({codes:'statecode',keyParameter:'countrycode',parseKey:function(id){return id.split(':')[0];}});PORTAL.MODELS.countyCodes=PORTAL.MODELS.cachedCodesWithKeys({codes:'countycode',keyParameter:'statecode',parseKey:function(id){var idArray=id.split(':');return idArray[0]+':'+idArray[1];}});PORTAL.MODELS.siteType=PORTAL.MODELS.cachedCodes({codes:'sitetype'});PORTAL.MODELS.organization=PORTAL.MODELS.cachedCodes({codes:'organization'});PORTAL.MODELS.sampleMedia=PORTAL.MODELS.cachedCodes({codes:'samplemedia'});PORTAL.MODELS.characteristicType=PORTAL.MODELS.cachedCodes({codes:'characteristictype'});PORTAL.MODELS.assemblage=PORTAL.MODELS.cachedCodes({codes:'assemblage'});var PORTAL=PORTAL||{};PORTAL.DataSourceUtils=(function(){"use strict";var that={};that.getCountsFromHeader=function(xhr,dataSources){
var formatCount=function(key){var countHeader=xhr.getResponseHeader(key);if(countHeader&&countHeader!=='0'){return $.formatNumber(countHeader,{format:'#,###',locale:'us'});}
...
result.total={sites:formatCount('Total-Site-Count'),results:formatCount('Total-Result-Count')};return result;};return that;}());var PORTAL=PORTAL||{};PORTAL.VIEWS=PORTAL.VIEWS||{};PORTAL.VIEWS.placeSelects=function(countryEl,stateEl,countyEl){var that={};that.getCountries=function(){var results=countryEl.select2('val');if(results.length===0){return['US'];}
